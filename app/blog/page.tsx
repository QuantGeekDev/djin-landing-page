  ...
  return (
    <div className="min-h-screen pt-32 sm:pt-40 pb-20 px-5 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <Link href="/" className="label text-muted hover:text-foreground transition-colors mb-8 inline-block">
          &larr; Back to Jinn
        </Link>

        <h1 className="heading-lg text-3xl sm:text-4xl md:text-5xl mb-4">Blog</h1>
        <p className="text-muted text-[15px] font-light mb-12 sm:mb-16 max-w-xl">
        ...
        <div className="flex flex-wrap gap-2 mb-12 sm:mb-16">
          {categories.map((cat) => (
            <span
              key={cat.slug}
              className="px-3 py-1.5 rounded-full border border-border text-[12px] text-muted/60 font-light"
            >
              {cat.name}
            </span>
          ))}
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-8">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="label text-accent-warm/60 text-[10px]">{post.category}</span>
                    <span className="text-muted/20 text-[10px]">&middot;</span>
                    <span className="label text-muted/30 text-[10px]">{post.readingTime}</span>
                  </div>
                ...
                <time className="text-[12px] text-muted/30 font-mono shrink-0 sm:mt-1">
