const categories = [
  ...
  { name: "Djin HoloBox", sub: "AI agent in a smart display", v: [1,1,1,1,1,1,1] },
];
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="hidden md:block overflow-hidden rounded-2xl border border-border">
          <table className="w-full text-sm">
            ...
            <tbody>
              {categories.map((cat) => {
                const isDjin = cat.name === "Djin HoloBox";
                return (
                  <tr key={cat.name} className={`border-b border-border last:border-0 ${isDjin ? "bg-accent-warm/[0.03]" : ""}`}>
                    <td className="px-6 py-5">
                      <p className={`text-[14px] ${isDjin ? "text-accent-warm font-normal" : "font-light"}`}>{cat.name}</p>
        ...
        <div className="md:hidden space-y-3">
          {categories.map((cat) => {
            ...
            return (
              <div key={cat.name} className={`rounded-xl border p-4 ${isDjin ? "border-accent-warm/30 bg-accent-warm/[0.03]" : "border-border"}`}>
                <div className="flex justify-between items-center">
                  <div>
                    <p className={`text-sm ${isDjin ? "text-accent-warm" : ""}`}>{cat.name}</p>
                    <p className="text-[11px] text-muted/40">{cat.sub}</p>
                  </div>
                  <span className={`font-mono text-sm ${isDjin ? "text-accent-warm" : "text-muted/40"}`}>{count}/7</span>
                </div>
                <div className="flex gap-1 mt-3">
                  {cat.v.map((val, i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full ${val ? (isDjin ? "bg-accent-warm/60" : "bg-foreground/20") : "bg-border"}`} />
                  ))}
                </div>
              </div>
            );
