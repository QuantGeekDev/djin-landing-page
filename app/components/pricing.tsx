...
import { Section, SectionHeader, CardGrid, Button } from "@/app/components/ui";
...
export default function Pricing() {
  return (
    <Section id="pricing" maxWidth="4xl">
      <SectionHeader
        label="Pricing"
        heading="Own the device. Choose how you run it."
        subtitle="Bring your own API keys, or let Jinn Cloud handle everything. The hardware is yours either way."
      />
          ...
          <div key={t.name} className={`p-6 md:p-8 flex flex-col ${t.highlight ? "bg-accent-warm/[0.03]" : "bg-background"}`}>
            ...
            {t.highlight ? (
              <PreorderButton
                batch="batch_2"
                source="pricing"
                className="w-full py-3 sm:py-2.5 rounded-button-rect text-[13px] font-medium transition-interactive bg-foreground text-background hover:bg-accent-warm hover:text-white"
              >
      ...
      {/* Guarantee */}
      <div className="mt-10 sm:mt-12 rounded-container border border-accent-warm/20 bg-accent-warm/[0.02] p-6 sm:p-8 text-center">
